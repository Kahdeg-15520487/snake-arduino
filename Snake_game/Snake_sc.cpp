...
Snake::Snake(UTFT scr)
{
    Init(L);
    for (int i = 0; i < n; i++) {
        Square a(cul.getx() + i * 10, cul.getx() + i * 10, scr);
        Insert_first(L, a);
        L->data.drawSq(dir);
    }
}
...
void
Insert_first(List &L, Square x) // Chen x vao vi tri dau tien trong danh sach
{
    Node * P;
